    ...
    <div className="container-wide">
      <div className="grid items-center gap-12 lg:gap-24 lg:grid-cols-2">
        <div className="relative">
          <div className="relative overflow-hidden shadow-[0_32px_100px_rgba(42,36,31,0.12),_0_0_0_1px_rgba(232,154,67,0.15),_0_0_0_4px_rgba(250,248,244,0.8),_inset_0_1px_0_rgba(255,255,255,0.1)] aspect-[4/5] md:aspect-[3/4] rounded-3xl bg-[var(--bg)] skeleton" />
        </div>
        <div className="space-y-8 md:space-y-10 max-w-3xl">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-widest text-[var(--accent)] mb-4 skeleton" />
            <h3 className="font-display text-3xl md:text-4xl lg:text-5xl font-normal text-[var(--primary)] leading-[1.15] skeleton" />
          </div>
          <p className="text-base md:text-lg leading-[1.8] text-[var(--primary)]/70 skeleton" />
          <div className="py-2 border-t border-b border-[var(--secondary)]/40">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-[var(--accent)] mb-3 skeleton" />
            <p className="font-display text-xl md:text-2xl text-[var(--primary)] italic leading-relaxed skeleton" />
          </div>
        </div>
      </div>
    </div>
...
SkeletonAbout.displayName = 'SkeletonAbout'

const ErrorAbout = memo(({ onRetry }) => (
  <section className="bg-[var(--bg)] px-6 md:px-12 lg:px-20 py-20 md:py-32">
    <div className="container-wide text-center py-12">
      <p className="font-display text-xl text-[var(--primary)]/60 mb-4">Unable to load about section</p>
...
ErrorAbout.displayName = 'ErrorAbout'
...
export const AboutSection = memo(() => {
  ...
  const loadAbout = useCallback(async () => {
    try {
      ...
      console.warn('[ABOUT SECTION] Failed to load:', err?.message)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  if (error) return <ErrorAbout onRetry={loadAbout} />
